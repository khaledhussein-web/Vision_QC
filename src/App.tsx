...
export type Screen = 'splash' | 'login' | 'register' | 'forgot-password' | 'home' | 'upload' | 'result' | 'chat' | 'history' | 'bookmarks' | 'profile' | 'payment' | 'admin-dashboard' | 'admin-images' | 'admin-image-detail' | 'admin-reports' | 'admin-access' | 'api-docs';

export interface UserData {
  id: number;
  name: string;
  email: string;
  role: 'admin' | 'annotator' | 'user';
  joinDate: string;
}
...
export default function App() {
  ...
  const handleLogin = (isAdminUser: boolean, userId: number) => {
  ...
  const handleAddUser = (user: Omit<UserData, 'id' | 'joinDate'>) => {
  ...
  const handleUpdateUser = (id: number, updates: Partial<UserData>) => {
