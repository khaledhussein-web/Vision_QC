...
import { ArrowLeft, Bookmark, ImageOff, Loader2, Trash2 } from 'lucide-react';
...
export default function BookmarksScreen({ navigate, userId }) {
  ...
  const emptyStateMessage = useMemo(() => {
    if (!userId) {
      return 'Please sign in to view your bookmarks.';
    }
    return 'No bookmarks yet. Save predictions from your history to see them here.';
  }, [userId]);
  ...
  useEffect(() => {
    ...
    const loadBookmarks = async () => {
      ...
      try {
        const response = await getHistory(userId, page, perPage);
        if (!isMounted) return;
        const historyItems = response.data || [];
        setBookmarks(historyItems.filter(isBookmarked));
        setTotalPages(response.total_pages || 1);
      } catch (err) {
        if (!isMounted) return;
        setError(err?.error || 'Unable to load bookmarks.');
      } finally {
        if (isMounted) {
          setLoading(false);
        }
  ...
  const handleRemove = async (predictionId) => {
    if (!userId || !predictionId) return;
    setRemovingId(predictionId);
    try {
      await toggleBookmark(predictionId, userId, 'remove');
      setBookmarks((prev) => prev.filter((item) => item.prediction_id !== predictionId));
    } catch (err) {
      setError(err?.error || 'Unable to remove bookmark.');
    } finally {
      setRemovingId(null);
    }
  };
      ...
      <div className="flex-1 p-6 space-y-6">
        ...
        {!loading && !error && bookmarks.length === 0 && (
          <div className="bg-white rounded-2xl p-6 shadow-md text-center">
            ...
            <h3 className="text-gray-900 mb-2">No bookmarks</h3>
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="pt-2">
                      <Button
                        variant="outline"
                        onClick={() => handleRemove(item.prediction_id)}
                        disabled={removingId === item.prediction_id}
                      >
                        <Trash2 className="w-4 h-4 mr-2" />
                        {removingId === item.prediction_id ? 'Removing...' : 'Remove Bookmark'}
                      </Button>
                    </div>
                  </div>
                </div>
