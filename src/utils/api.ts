...
const mockPredictions = new Map<number, any>();
const mockImages = new Map<number, any>();
const mockChatHistory = new Map<number, any[]>();
let nextImageId = 1;
...
export async function uploadImage(
  ...
  metadata?: any
): Promise<UploadImageResponse & { prediction?: PredictionDetails }> {
  await delay(1500);
 ...
 */
export async function getPredictionDetails(predictionId: number): Promise<PredictionDetails> {
  await delay(500);
...
export async function getHistory(
  ...
  const userPredictions: HistoryItem[] = [];
